...
struct ClientListView: View {
        ...
        List {
            ForEach(viewModel.filteredClients) { client in
                ...
                .listRowBackground(
                    RoundedRectangle(cornerRadius: 10)
                        ...
                        .padding(.vertical, 0) // Vertikales Padding entfernt
                )
                .listRowSeparator(.hidden)
                .listRowInsets(EdgeInsets(top: 3, leading: 13, bottom: 3, trailing: 13)) // Abstand auf 0 gesetzt
            ...
            if viewModel.isLoading {
                ProgressView()
                    ...
                    .listRowInsets(EdgeInsets(top: 0, leading: 10, bottom: 0, trailing: 10)) // Abstand auf 0 gesetzt
            }
        }
    ...
    private func clientRow(for client: Client) -> some View {
        VStack(spacing: 0) {
            // Feiner weißer Strich als obere Trennlinie
            Rectangle()
            ...
            HStack(spacing: 12) {
                ...
                VStack(alignment: .leading, spacing: 2) { // Reduziertes Spacing
                    Text("\(client.vorname) \(client.name)")
                        .font(.system(size: 14, weight: .bold, design: .rounded))
                        .foregroundColor(.white)

                    VStack(alignment: .leading, spacing: 1) { // Reduziertes Spacing
