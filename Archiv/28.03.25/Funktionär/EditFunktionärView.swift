...
struct EditFunktionärView: View {
    ...
    var body: some View {
        NavigationView {
            Form {
                ...
                Section(header: Text("Profilbild")) {
                    ...
                    TextField("Oder Bild-URL eingeben", text: $imageURL)
                        ...
                        .background(Color.yellow.opacity(0.3)) // Debugging: Hintergrundfarbe hinzufügen, um Sichtbarkeit zu überprüfen
            ...
            .onChange(of: selectedPhoto) { newValue in
                print("EditFunktionärView - SelectedPhoto geändert: \(newValue != nil ? "Foto ausgewählt" : "Kein Foto")")
    ...
    private var profileImagePicker: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            TextField("Oder Bild-URL eingeben", text: $imageURL)
                .autocapitalization(.none)
                .keyboardType(.URL)
                .textFieldStyle(RoundedBorderTextFieldStyle()) // Stil hinzufügen, um die Sichtbarkeit zu verbessern
    ...
    private func loadSelectedImage() async {
        guard let photoItem = selectedPhoto else {
            print("EditFunktionärView - Kein Foto zum Laden ausgewählt")
            await MainActor.run {
            ...
            if let data = try await photoItem.loadTransferable(type: Data.self),
               let image = UIImage(data: data) {
                await MainActor.run {
                    ...
                    print("EditFunktionärView - Bild erfolgreich geladen: \(String(describing: image))")
                }
            } else {
                await MainActor.run {
                    self.profileImage = nil
